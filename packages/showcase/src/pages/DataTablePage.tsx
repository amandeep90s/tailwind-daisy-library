		...
		<div className="space-y-4">
			<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
				<div>
					...
					<p className="text-sm text-base-content/70">{order.shippingAddress}</p>
				</div>
				<div>
...
export function DataTablePage() {
	...
	return (
		<ComponentPage title="Sortable DataTable" description="A feature-rich data table with sorting, expandable rows, and pagination.">
			<ShowcaseSection
			...
			>
				<div className="space-y-2">
					<p className="text-sm text-base-content/70">Current page: {paginationPage}</p>
					<SortableDataTable
						data={extendedUsers}
						columns={userColumns}
						getRowKey={(user) => user.id}
			...
			>
				<div className="space-y-8">
					<div>
						<p className="text-sm font-medium mb-2">Pagination at Top</p>
						<SortableDataTable
							data={extendedUsers}
					...
					<div>
						<p className="text-sm font-medium mb-2">Pagination at Both</p>
						<SortableDataTable
							data={extendedUsers}
							columns={userColumns.slice(0, 4)}
							getRowKey={(user) => user.id}
			...
			<ShowcaseSection title="Default Expanded Rows" description="Table with some rows expanded by default.">
				<SortableDataTable
					data={orders}
					columns={orderColumns}
					getRowKey={(order) => order.id}
			...
			>
				<SortableDataTable
					...
					columns={[
						...
						{ key: "category", header: "Category", sortable: true, headerAlign: "center", cellAlign: "center" },
						{
							key: "price",
							header: "Price",
							sortable: true,
			...
			>
				<div className="space-y-6">
					<div>
						<p className="text-sm font-medium mb-2">Custom Sort Icons</p>
						<SortableDataTable
							data={users.slice(0, 4)}
							columns={userColumns.slice(0, 4)}
							getRowKey={(user) => user.id}
							sortAscIcon={<ChevronUpIcon className="w-4 h-4 text-primary" />}
							sortDescIcon={<ChevronDownIcon className="w-4 h-4 text-primary" />}
							sortBothIcon={<ChevronUpDownIcon className="w-4 h-4 text-base-content/40" />}
							zebra
						/>
					</div>
					<div>
						<p className="text-sm font-medium mb-2">Custom Expand Icons</p>
						<SortableDataTable
							...
							expandIcon={(expanded) =>
								expanded ? (
									<MinusCircleIcon className="w-5 h-5 text-error" />
								) : (
			...
			<ShowcaseSection title="Empty State" description="Table displays an empty message when there's no data.">
				<SortableDataTable
					data={[]}
					columns={userColumns}
					getRowKey={(user) => user.id}
			...
			<PropsTable
				props={[
					...
					},
					{
						...
						description: "Column definitions including key, header, sortable flag, and render function",
					},
					{
						name: "getRowKey",
