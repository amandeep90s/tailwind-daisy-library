...
const SortIcon = ({
  ...
  if (direction === "both") {
    return (
      <div className="flex flex-col -space-y-1">
        <svg className={clsx("h-3 w-3", activeClass)} fill="currentColor" viewBox="0 0 24 24">
  ...
  if (direction === "asc") {
    return (
      <svg className={clsx("h-4 w-4", activeClass)} fill="currentColor" viewBox="0 0 24 24">
        <path d="M7 14l5-5 5 5H7z" />
      </svg>
    );
  }
...
const ExpandIcon = ({ expanded }: { expanded: boolean }) => (
  <svg
    className={clsx("h-5 w-5 transition-transform duration-200", expanded && "rotate-180")}
    fill="none"
    stroke="currentColor"
    viewBox="0 0 24 24"
  >
      ...
      return (
        <div className={clsx("flex items-center justify-between", paginationClassName)}>
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-base-content/70 text-sm">of {recordRange.total} records</span>
          </div>
