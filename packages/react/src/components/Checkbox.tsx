...
export type CheckboxVariant = "primary" | "secondary" | "accent" | "info" | "success" | "warning" | "error";
	...
	({ variant, size = "md", label, indeterminate, className, id, ...props }, ref) => {
		...
		const checkbox = (
			<input
				...
				id={checkboxId}
				type="checkbox"
				className={clsx("checkbox", variant && variantClasses[variant], sizeClasses[size], className)}
				{...props}
			/>
		);
